...
test('ASEAN topic filters existing signals and exposes range-based forecast questions separately', () => {
  ...
  const aseanRefreshRouteSource = readSource('src/app/api/v1/world/asean/refresh/route.ts');
  const aseanRefreshJobSource = readSource('src/lib/world/asean-refresh-job.ts');
  const aseanAbortSource = readSource('src/lib/world/asean-abort.ts');
  ...
  assert.match(routeSource, /cacheOnly: true/);
  assert.match(routeSource, /refresh_job/);
  ...
  assert.match(demoClientSource, /后台更新中/);
  assert.match(demoClientSource, /timeZone: 'Asia\/Shanghai'/);
  ...
  assert.match(aseanResearchRouteSource, /phase: '准备上下文'/);
  assert.match(aseanRefreshRouteSource, /status: status\.reused \? 200 : 202/);
  assert.match(aseanRefreshRouteSource, /cancelAseanRefresh/);
  assert.match(routeSource, /\/api\/v1\/world\/asean\/refresh/);
  assert.doesNotMatch(routeSource, /startAseanRefresh/);
  assert.match(aseanRefreshJobSource, /asean-refresh\.lock/);
  assert.match(aseanRefreshJobSource, /createLinkedTimeoutSignal/);
  assert.match(aseanRefreshJobSource, /workerHostname/);
  assert.match(aseanAbortSource, /AbortController/);
  assert.match(aseanAbortSource, /raceAseanAbort/);
  assert.match(aseanAbortSource, /Promise\.race/);
  assert.match(aseanAbortSource, /rename/);
  assert.doesNotMatch(aseanPageDataSource, /Promise\.race/);
