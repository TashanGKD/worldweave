...
test('GitHub Actions deploys main with the configured server secrets', () => {
  ...
  assert.match(deploy, /Prepare SSH private key/)
  assert.match(deploy, /base64\.b64decode/)
  assert.match(deploy, /ssh-keygen -y -f \.deploy_key/)
  assert.match(deploy, /key_path: \/github\/workspace\/\.deploy_key/)
