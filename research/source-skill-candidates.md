...
| public-apis | GitHub | https://github.com/public-apis/public-apis | unstable | public API directory / source discovery | GitHub README raw 可读，当前实测 raw README 200 | 大量公共 API 分类目录，覆盖金融、新闻、天气、政府、科学、交通、社交、加密等主题 | README 是人工维护的 API 索引，不是单一实时 feed；适合作为批量发现 API 的上游目录，再按主题拆成可验证 endpoint | mixed | high | 目录规模大、质量不一，不能整体接入；必须按 API 逐条校验授权、频率、稳定性和主题相关性 | 先抽金融、新闻、政府/公开数据、天气/灾害、交通/航运、加密几个板块，生成 endpoint 级候选 |
| awesome-ai-in-finance | GitHub | https://github.com/georgezouq/awesome-ai-in-finance | unstable | finance AI resource directory / method discovery | GitHub README raw 可读，当前实测 raw README 200 | 金融 AI 研究、交易、预测、风控、工具和代码资源 | 资源目录更偏方法与项目索引，能反向发现数据集、行情/新闻工具和金融预测项目；不是直接新闻或行情源 | mixed | medium | 条目偏研究和代码，实时信号密度低于 API/RSS 目录；需要二次筛出真正有数据入口的项目 | 作为金融预测方法层和候选项目池保留，优先筛能暴露 API/RSS/dataset 的条目 |
...
| TrendRadar | GitHub | https://github.com/sansan0/TrendRadar | direct | hotspot discovery / multi-platform trend aggregation | 官方 GitHub README、配置、MCP server 与源码可读 | 今日头条、百度热搜、华尔街见闻、澎湃、B站热搜、财联社热门、凤凰网、贴吧、微博、抖音、知乎、Hacker News、Yahoo Finance RSS、自定义 RSS | README 与源码已确认其热榜数据默认来自 `newsnow.busiyi.world/api/s` 多平台热点 API，另支持独立 RSS 抓取、统一搜索、趋势分析、MCP 查询与 AI 翻译/筛选流程 | mixed | high | 聚合层很强，但热点榜本身不是原始报道；需要和原文跳转、可信度和去重规则一起使用 | 很适合作为“热点发现层 + RSS 汇聚层”补充样本，尤其适合弱信号/中文平台趋势捕捉 |
| newsnow | GitHub | https://github.com/ourongxing/newsnow | direct | hotlist API / multi-platform trend backend | 官方 GitHub repo、TrendRadar 依赖说明与 live API 可读 | 多平台热榜与热点列表 API | TrendRadar README 与 fetcher 已明确默认调用 `https://newsnow.busiyi.world/api/s`；当前已实测 `?id=baidu&latest` 返回 200，可视为真实热点 API 上游 | api | high | 仍是聚合热榜层，不是原始新闻源；平台覆盖随上游适配变动 | 可作为中文与泛平台热点入口层的真实上游，适合配合原文回跳和跨平台比较 |
| ShunyaNet Sentinel | GitHub | https://github.com/EverythingsComputer/ShunyaNet-Sentinel | direct | rss monitoring / local-llm triage / alert workflow | 官方 GitHub README、默认 RSS 列表、prompt 与主程序源码可读 | Default_long 世界新闻 RSS 包、India regional 示例、Iran conflict 示例、default prompt、summary-only prompt、Slack webhook 告警工作流 | README 与源码已确认其核心是 RSS 轮询、去重、chunk、prompt 驱动过滤、Slack 告警与 bulk trend analysis；当前 raw README 与默认 RSS 列表均可直连，且 bundled feed list 覆盖 BBC/AP/CNN/USGS/GDACS/Reddit/Bluesky/Newswise 等真实公开源 | mixed | high | 它本身不是新原始信源，而是“RSS 组织与本地分析工作流”；其中部分 bundled feed 属于社交/聚合层，仍需按我们规则筛源 | 很适合作为“如何组织 RSS 清单 + 如何把本地/自托管 LLM 接进持续监测”的高价值样本，也适合补充地区化 feed list 模板 |
...
| apilayer-marketstack | APILayer official repo | https://github.com/apilayer/marketstack | direct | financial API / cross-market quote infrastructure | 官方 GitHub README、官方文档可读 | marketstack、stocks、intraday、historical data、tickers、exchanges、currencies | 官方 README 已明确 `125,000+ stock tickers`、`72+ exchanges`、EOD / intraday / historical / tickers / exchanges / currencies / timezones；当前已实测 `https://marketstack.com/documentation` 返回 200、`https://api.marketstack.com/v1/tickers` 返回 401，证明文档与 API 根链路存在 | api | high | 需要 API key；更像底层行情基础设施，不直接提供世界事件语义层 | 适合作为市场子世界的底层行情补充池，后续可与 Signal Arena / Alpha Vantage 做对照接入 |
| apilayer-mediastack | APILayer official repo | https://github.com/apilayer/mediastack | direct | news API / global aggregated news | 官方 GitHub README、官方文档可读 | mediastack、live news、historical news、7,500+ sources、50+ countries、13 languages | 官方 README 已明确全球新闻聚合能力与 `7,500+ sources / 50+ countries / 13 languages`；当前已实测 `https://mediastack.com/documentation` 返回 200、`https://api.mediastack.com/v1/news` 返回 401，证明文档与 API 根链路存在 | api | high | 依赖 API key，且属于聚合新闻层，不适合作为主世界核心锚点 | 适合作为国际新闻补充层与长尾新闻覆盖池，后续应配合去重、原始来源回跳和可信度规则 |
...
| apilayer-marketstack | APILayer official repo | partially_verified | 官方 GitHub README 与官方文档已确认 marketstack 覆盖 `125,000+ tickers`、`72+ exchanges`、EOD / intraday / historical / tickers / exchanges / currencies / timezones；当前实测 `marketstack.com/documentation` 为 200、`api.marketstack.com/v1/tickers` 为 401，说明文档与受鉴权 API 根链路存在 | yes | named_upstreams | market-signal | direct-source | p1 |
| apilayer-mediastack | APILayer official repo | partially_verified | 官方 GitHub README 与官方文档已确认 mediastack 覆盖 `7,500+ sources`、`50+ countries`、`13 languages` 的 live/historical news；当前实测 `mediastack.com/documentation` 为 200、`api.mediastack.com/v1/news` 为 401，说明文档与受鉴权 API 根链路存在 | yes | named_upstreams | hotspot-discovery | aggregator-layer | p1 |
| TrendRadar | GitHub | partially_verified | 官方 README、MCP FAQ、fetcher 与 config 已确认热榜依赖 `newsnow.busiyi.world/api/s`，并支持 RSS、统一搜索、趋势分析与 MCP 查询；当前实测 GitHub Pages 演示页 200 | yes | named_upstreams | hotspot-discovery | aggregator-layer | p1 |
| newsnow | GitHub | partially_verified | TrendRadar README 与 fetcher 已确认其为多平台热榜默认上游，当前实测 `https://newsnow.busiyi.world/api/s?id=baidu&latest` 返回 200，说明真实 API 链路存在 | yes | named_upstreams | hotspot-discovery | direct-source | p1 |
| ShunyaNet Sentinel | GitHub | partially_verified | 官方 README、主程序与默认数据源列表已确认其围绕 RSS 轮询、去重、chunk、Slack webhook、LM Studio / Ollama / OpenAI 兼容 LLM 接口运行；当前实测 GitHub repo、raw README、Default_long-v1、Iran_List_V1 与 default_prompt 均可直连 | yes | named_upstreams | hotspot-discovery | tooling-reference | p1 |
...
| public-apis | GitHub | partially_verified | raw README 已确认其为人工维护的公共 API 目录，适合从中拆金融、新闻、政府、天气、交通、加密等 endpoint；当前只作为发现入口，不直接入实时信号 | maybe | broad_pattern | hotspot-discovery | tooling-reference | p1 |
| awesome-ai-in-finance | GitHub | partially_verified | raw README 已确认其为金融 AI 研究、工具与代码资源目录，适合反向发现金融预测/交易/风控数据项目；当前不视作直接信源 | maybe | broad_pattern | market-signal | tooling-reference | p2 |
